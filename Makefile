...
lint:  ## run python linter with ruff
	python -m ruff check airflow_priority
...
annotate:  ## run python type annotation checks with mypy
	python -m mypy ./airflow_priority
...
test:  ## run python tests
	python -m pytest -v airflow_priority/tests --junitxml=junit.xml

coverage:  ## run tests and collect test coverage
	python -m pytest -v airflow_priority/tests --junitxml=junit.xml --cov=airflow_priority --cov-branch --cov-fail-under=50 --cov-report term-missing --cov-report xml
